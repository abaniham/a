...
export function createXPGraph(transactions) {
    console.log("Full transaction item:", JSON.stringify(transactions[0], null, 2));
    ...
    const graphData = calculateTotalXP(transactions);
    ...
    svg += `
        <path d="${pathD}" fill="none" stroke="#c7962c" stroke-width="3" />
    `;
    ...
    graphData.forEach(d => {
        ...
        svg += `
        <circle 
  cx="${x}" cy="${y}" r="5" fill="#815400" opacity="0.3" class="xp-point"
  data-project="${d.project}" 
  data-date="${new Date(d.date).toLocaleDateString()}" 
  data-xp="${d.cumulativeXP}" 
/>
  `;
    });
    ...
    // Tooltip behavior for XP points
const points = document.querySelectorAll('.xp-point');

points.forEach(point => {
    point.addEventListener('mouseenter', (e) => {
        const project = point.getAttribute('data-project');
